...
public class CharacterUtils {
    ...
    private final static String TAG = CharacterUtils.class.getSimpleName();

    private final static int[] PIN_YIN_VALUE = new int[]{-20319, -20317, -20304, -20295, -20292, -20283, -20265, -20257, -20242, -20230, -20051, -20036, -20032,
    ...
    private final static String[] PIN_YIN_STR = new String[]{"a", "ai", "an", "ang", "ao", "ba", "bai", "ban", "bang", "bao", "bei", "ben", "beng", "bi", "bian",
    ...
    private static int getChineseCharASCII(byte[] src){

        int highByte = 256 + src[0];
        int lowByte = 256 + src[1];
        int asc = (256 * highByte + lowByte) - 256 * 256;
        return asc;
    }

    private static String getPinYin(int ascii){

        for (int i = (PIN_YIN_VALUE.length - 1); i >= 0; i--) {
            if (PIN_YIN_VALUE[i] <= ascii) {
                return PIN_YIN_STR[i];
            }
        }
     ...
     * @return
     */
    private static int getASCII(String chr) {
        int asc = 0;
        try {
            byte[] bytes = chr.getBytes("gb2312");
    ...
    public static String transferToPinYin(String str) {
        ...
        String pinYin = getPinYin(ascii);
        return (pinYin == null ? str : pinYin);
    }
     ...
     * @return
     */
    public static String transferChineseChrToPinYin(String word) {

        if(TextUtils.isEmpty(word)){
            return null;
        }

        String character, spelling;
        StringBuilder buffer = new StringBuilder();

        for (int i = 0; i < word.length(); i++) {
            character = word.substring(i, i + 1);
            spelling = transferToPinYin(character);
